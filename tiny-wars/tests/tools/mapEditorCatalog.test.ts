...
describe('mapEditorCatalog sync', () => {
  ...
  it('exports bomb tower bomber paths from bomb_fish idle sheets', () => {
    ...
    expect(constants.compositeLayerOffsets.wood_tower?.platform).toEqual({ x: 1, y: 143 })
    expect(constants.compositeLayerOffsets.wood_tower?.bomber).toEqual({ x: 2, y: 216 })
  ...
  it('exports king tower layout from GameConstants', () => {
    ...
    expect(layout.kingLogicCol).toBe(8.5)
    expect(layout.playerKingRow).toBe(29)
    expect(layout.botKingRow).toBe(2)
    expect(layout.playerTowerRow).toBe(25)
    expect(layout.botTowerRow).toBe(6)
    expect(layout.kingSpriteCenterOffsetX).toBe(0)
    expect(layout.bridgeCenterCol).toBe(9)
    expect(layout.bridgeSpan).toBe(3)
    expect(layout.leftBridgeStart).toBe(2)
    expect(layout.rightBridgeStart).toBe(13)
  })
  ...
  it('map-editor.html default spawn zones match buildClashStyleSpawnZones', () => {
    ...
    expect(onDisk).toContain('"0,13": "left"')
    expect(onDisk).toContain('"17,13": "right"')
  })
})
