...
describe('Arena 26 balance (level 14)', () => {
  ...
  it('scales key cards to level 14', () => {
    ...
    expect(CARD_DEFINITIONS.mega_minion!.elixirCost).toBe(7)
    expect(CARD_DEFINITIONS.mega_minion!.displayName).toBe('Bear')
    expect(CARD_DEFINITIONS.mega_minion!.stats!.maxHp).toBe(5288)
    expect(CARD_DEFINITIONS.mega_minion!.stats!.damage).toBe(355)
    expect(CARD_DEFINITIONS.mega_minion!.stats!.attackRate).toBeCloseTo(1 / 1.7, 5)
    expect(CARD_DEFINITIONS.mega_minion!.stats!.attackRange).toBe(1.2)
    expect(CARD_DEFINITIONS.mega_minion!.stats!.splashRadius).toBe(1.3)
