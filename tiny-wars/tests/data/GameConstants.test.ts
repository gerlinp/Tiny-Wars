...
describe('Arena 26 balance (level 14)', () => {
  ...
  it('scales key cards to level 14', () => {
    ...
    expect(CARD_DEFINITIONS.villagers!.stats!.maxHp).toBe(216)
    expect(CARD_DEFINITIONS.villagers!.stats!.damage).toBe(203)
    ...
    expect(CARD_DEFINITIONS.spider!.stats!.spawnMinionCardId).toBe('spiderling')
    expect(CARD_DEFINITIONS.paddle_shark!.elixirCost).toBe(5)
    expect(CARD_DEFINITIONS.paddle_shark!.deployCount).toBe(5)
    expect(CARD_DEFINITIONS.paddle_shark!.stats!.maxHp).toBe(915)
    expect(CARD_DEFINITIONS.paddle_shark!.stats!.damage).toBe(254)
    expect(CARD_DEFINITIONS.paddle_shark!.stats!.attackRate).toBeCloseTo(1 / 1.4, 5)
    expect(CARD_DEFINITIONS.paddle_shark!.stats!.attackRange).toBe(0.7)
    expect(CARD_DEFINITIONS.paddle_shark!.stats!.speed).toBe(crSpeedToCellsPerSec(CR_SPEED.medium))
