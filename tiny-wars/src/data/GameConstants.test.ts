...
describe('Arena 26 balance (level 14)', () => {
  ...
  it('scales key cards to level 14', () => {
    ...
    expect(CARD_DEFINITIONS.rogue!.elixirCost).toBe(3)
    expect(CARD_DEFINITIONS.rogue!.stats!.maxHp).toBe(1200)
    expect(CARD_DEFINITIONS.rogue!.stats!.damage).toBe(257)
    expect(CARD_DEFINITIONS.rogue!.stats!.attackRate).toBeCloseTo(1 / 1.0, 5)
    expect(CARD_DEFINITIONS.rogue!.stats!.attackRange).toBe(0.75)
    expect(CARD_DEFINITIONS.rogue!.stats!.speed).toBe(crSpeedToCellsPerSec(CR_SPEED.fast))
    expect(CARD_DEFINITIONS.rogue!.stats!.firstHitDamageMultiplier).toBe(2)
